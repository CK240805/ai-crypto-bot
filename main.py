...
NVIDIA_API_KEY = os.getenv("NVIDIA_API_KEY")  # Changed from DeepSeek
NVIDIA_MODEL_NAME = os.getenv("NVIDIA_MODEL_NAME", "deepseek-ai/deepseek-v3") # Default to V3
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN")
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID")
MAX_CAPITAL = 100.00  # Hard cap
MCP_SERVER_URL = "YOUR_TRADER_DEV_PUBLIC_URL_HERE" # Replace later

# --- INITIALIZE APIS ---
# SWITCHED TO NVIDIA NIM ENDPOINT
...
# --- FASTAPI APP (To keep Replit alive) ---
app = FastAPI()
...
async def generate_strategy_from_deepseek():
    ...
    response = client.chat.completions.create(
        model=NVIDIA_MODEL_NAME, # Using the NVIDIA NIM model
...
async def run_backtest_on_trader_dev(strategy_params):
    # PLACEHOLDER: Replace this with your actual Trader.dev API call later
    # payload = {"strategy": strategy_params, "symbol": "EUR_USD"}
...
async def find_amazing_strategy_loop():
    while True:
        print("🔄 Optimization Loop: Generating strategy via NVIDIA NIM...")
        ...
        if is_amazing:
            ...
            await telegram_bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"🚀 Bot executed trade based on AI strategy!\nResult: {execution}")
        else:
            print("❌ Not amazing. Asking DeepSeek (via NVIDIA) to refine...")
...
async def send_morning_report():
    ...
    await telegram_bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"🌅 Morning Market Analysis:\n\n{response.choices[0].message.content}")

async def send_night_performance():
    await telegram_bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"🌙 Night Performance Profile:\nToday's P&L: $ -2.50 (Simulated - Fetch via OANDA API)\nOpen Positions: 1")
